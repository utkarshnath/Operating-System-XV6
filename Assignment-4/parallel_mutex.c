...
Keys get lost when two/more threads try to update the same bucket i.e line: e->next = table[i] and/or table[i] = e; in insert function.

Time comparison:

Without mutex : using 1 thread : 9.207 secs
Without mutex : using 2 thread : 5.456 secs
Without mutex : using 4 thread : 3.038 secs
Without mutex : using 8 thread : 2.936 secs
Using Mutex : using 1 thread : 9.213 secs
Using Mutex : using 2 thread : 5.661 secs
Using Mutex : using 4 thread : 3.049 secs
Using Mutex : using 8 thread : 2.947 secs

Part 3:

No retreiving item does not require a lock. The retreiving operation is already parallelized. Its only during the insertion phase, keys are getting lost because of multiple threads accessing the crirtical section (i.e line: e->next
= table[i] and/or table[i] = e;) at the same time. Hence, lock was needed to make sure only one thread acceses the critical section at a time. No keys are lost during retreival of keys and hence no lock is requried.
