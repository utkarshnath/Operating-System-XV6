...
Part 2:
Yes using spinlocks it takes more time i.e there is more overhead than using mutex. Below are the timing differences:

For Mutex : using 1 thread : 9.213 secs
For Mutex : using 2 thread : 5.661 secs
For Mutex : using 4 thread : 3.049 secs
For Mutex : using 8 thread : 2.947 secs
For Spinlock : using 1 thread : 9.341 secs
For Spinlock : using 2 thread : 5.752 secs
For Spinlock : using 4 thread : 3.088 secs
For Spinlock : using 8 thread : 3.007 secs

The more time required for spinlocks can be explained by their implementation. The thread blocks and consumes CPU time waiting to acquire the lock (busy waiting), whereas in case of mutex they thread goes to sleep for some amount of time if lock is not available, thus yielding the CPU for other threads if any. Hence mutex require less time than spinlock in the above program. 
